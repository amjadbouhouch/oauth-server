...
export class Logger {
  ...
  timeStart: number;
  timeEnd: number | null;
  timeElapsedMs: string | null;

  constructor(appName = 'Server', debugOn = true) {
   ...
   * @returns
   */
  debug(msg: string, op: any = '') {
    if (this.debugOn) {
      this.timeEnd = Date.now();
      this.timeElapsedMs = this.getFormatTime(Math.floor(this.timeEnd - this.timeStart));
      console.debug('[' + this.getDataTime() + '] [' + this.appName + '] ' + msg, util.inspect(op, options), this.timeElapsedMs);
      this.timeStart = Date.now();
    }
  }
  ...
  info(msg: string, op: any = '') {
    console.info('[' + this.getDataTime() + '] [' + this.appName + '] ' + colors.green(msg), util.inspect(op, options));
  }
  ...
  warn(msg: string, op: any = '') {
    console.info('[' + this.getDataTime() + '] [' + this.appName + '] ' + colors.yellow(msg), util.inspect(op, options));
  }
  ...
  error(msg: string, op: any = '') {
    console.info('[' + this.getDataTime() + '] [' + this.appName + '] ' + colors.red(msg), util.inspect(op, options));
  }
  ...
  static getLogger(appName = 'Server', debugOn = true) {
    return new Logger(appName, debugOn);
  }
}
